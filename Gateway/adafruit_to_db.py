...
DB_SSL = (os.getenv("DB_SSL") or "").strip().lower() in {"1", "true", "yes"}

if DATABASE_URL and ("<DB_PASSWORD>" in DATABASE_URL or "[YOUR-PASSWORD]" in DATABASE_URL):
    raise ValueError("DATABASE_URL vẫn đang dùng placeholder mật khẩu. Hãy thay bằng mật khẩu DB thật.")
...
def get_connection():
    if DATABASE_URL:
        dsn = DATABASE_URL
        if "sslmode=" not in dsn and DB_SSL:
            sep = "&" if "?" in dsn else "?"
